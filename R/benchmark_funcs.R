...
opt_sol <- function(opt) "sol" %in% names(opt) && opt$sol
...
f6.sols <- rastrigin.sols <- function(xmin, xmax) {
  ...
  xmaxi <- floor(xmax)
  if (n == 1) {
    return(matrix(xmini[n]:xmaxi[n], xmaxi[n] - xmini[n] + 1, 1, byrow = TRUE))
  }
...
f7.sols <- griewank.sols <- function(xmin, xmax, odd = FALSE) {
  ...
  if (n == 1) {
    x0 <- xmini[n] + if (odd) !xmini[n] %% 2 else xmini[n] %% 2
...
f7.num_sols <- griewank.num_sols <- function(d, xmin, xmax) {
  ...
  num_evens <- function(d) {
    floor(xmax[d] / (2 * pi * sqrt(d))) -
      ceiling(xmin[d] / (2 * pi * sqrt(d))) +
      1
  }
  num_odds <- function(d) {
    floor(xmax[d] / (2 * pi * sqrt(d)) + 0.5) -
...
ackley <- function(x, opt = NA) {
  ...
  -20 *
    exp(-0.2 * sqrt(sum(x^2) / n)) -
    exp(sum(cos(2 * pi * x)) / n) +
    20 +
    exp(1)
}
